...
const listExercise = `-- name: ListExercise :one
SELECT exercise_id, exercise_name
FROM gowebapp.exercises
WHERE exercise_id = $1
`

func (q *Queries) ListExercise(ctx context.Context, exerciseID int64) (GowebappExercise, error) {
	row := q.db.QueryRowContext(ctx, listExercise, exerciseID)
	var i GowebappExercise
	err := row.Scan(&i.ExerciseID, &i.ExerciseName)
	return i, err
}
